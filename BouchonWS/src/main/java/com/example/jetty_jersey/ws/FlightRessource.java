...
public class FlightRessource {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/book/passenger/waiting")
	public List<InformationReservation> getBookingForPassenger(@HeaderParam("token") String token) {
		if (JwTokenHelper.getInstance().isTokenInvalid(token)
				|| !JwTokenHelper.getInstance().getUserType(token).equals("passenger")) {
			return null;
		}
		return daoReservation.getReservationForPassenger(JwTokenHelper.getInstance().getIdFromToken(token), "0");
	}
	
	@POST
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/book/passenger/confirmed")
	public List<InformationReservation> getBookingForPassengerConfirmed(@HeaderParam("token") String token) {
		if (JwTokenHelper.getInstance().isTokenInvalid(token)
				|| !JwTokenHelper.getInstance().getUserType(token).equals("passenger")) {
			return null;
		}
		return daoReservation.getReservationForPassenger(JwTokenHelper.getInstance().getIdFromToken(token), "1");
	}
