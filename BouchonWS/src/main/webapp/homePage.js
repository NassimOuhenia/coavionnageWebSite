...
function afterSearch(listF) {
	...
	for(i=0;i<listF.length;i++){
		...
		//alert(listF[0].departureAirport+" "+listF[0].arrivalAirport+" "+listF[0].price);
	}
}
...
$(function(){
	$("#search").click(function(){
		...
		//alert(data.typeLocal+" "+data.typeTravel+" "+data.date+" "+data.departure+" "+data.arrival);
    });
});
...
$(function(){
	$("#postflight").click(function(){
		...
		//alert(data.typeflight+" "+data.departureAirport+" "+data.arrivalAirport+" "+data.date+" "+data.timep+" "
	...
	$("#createUser").click(function(){
		if(VerifFormSingup()){
			if($('#formsignup input[name="type"]:checked').val()=="pilot"){
				...
				getServerData(urlPostPilot, afterPostUser, 'post', formsignupToJSON());
			}else if($('#formsignup input[name="type"]:checked').val()=="passenger"){
				$("#formsignup .error-form").text("");
				getServerData(urlPostPassenger, afterPostUser, 'post', formsignupToJSON());
			}else{
	...
	$("#connectuser").click(function(){
		if(VerifFormSingin()){
			if($('#formsignin input[name="type"]:checked').val()=="pilot"){
				...
				type="pilot";
				getServerData(urlLogPilot, afterLoginUser, 'post', formlogToJSON());
			}else if($('#formsignin input[name="type"]:checked').val()=="passenger"){
				$("#formsignin .error-form").text("");
				type="passenger";
				getServerData(urlLogPassenger, afterLoginUser, 'post', formlogToJSON());
			}else{
...
$(function(){
	$("#logOut").click(function(){
		...
		$("#lin").css("display","inline");
		$("#sin").css("display","inline");
		profil=null;
	});
});
